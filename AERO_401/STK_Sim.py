...
class STK_Simulation:
    ...
    def Target_Loader(self,Target_Load_Option,Num_Targets):
        ...
        for i in range(Num_Targets):
            if Target_Load_Option == 0:
                ...
                lon = float(np.linspace(-90,90,Num_Targets)[i])
                lat = float(np.linspace(-90,90,Num_Targets)[i])
            
            print("{:.4f}\t:+t{:.4f}\t|\t{:.4f}".format(i+1,lon,lat))
    ...
    def Satellite_Loader(self,Satellite_Load_Option=0,Num_Satellites=12,per_range=(100,1000),ago_range=(100,1000),
        ...
        Print_Spacing()
        print(" sat##\t|\t  per\t\t|\t  ago\t\t|\t  inc\t|\t  aop\t|\t  asc\t|\t  loc")
        self.satellites = {}
        ...
        for i in range(Num_Satellites):
            if Satellite_Load_Option == 0:
                ...
                perigee_altitude = np.linspace(per_range[0],per_range[1],12)[i]
                apogee_altitude = np.linspace(ago_range[0],ago_range[1],12)[i]
                inclination = np.linspace(aop_range[0],aop_range[1],12)[i]
                argument_of_perigee = np.linspace(aop_range[0],aop_range[1],12)[i]
                ascending_node = np.linspace(asc_range[0],asc_range[1],12)[i]
                location = np.linspace(loc_range[0],loc_range[1],12)[i]

            print("{:.4f}\t|\t{:.4f}\t|\t{:.4f}\t|\t{:.4f}\t|\t{:.4f}\t|\t{:.4f}\t|\t{:.4f}".format(i+1,perigee_altitude,apogee_altitude,inclination,argument_of_perigee,ascending_node,location))
    ...
    def plot_FOM(self,FOM):
        ...
        for j in self.targets:
            ...
            targetCoverageFOM.SetDefinitionType(FOM)
            targetCoverageFOM.Definition.SetComputeType(AgEFmCompute.eTotal)
            
            DP = targetCoverage.DataProviders.Item('FOM by Time').Exec(self.root.CurrentScenario.StartTime, self.root.CurrentScenario.StopTime, 15)
