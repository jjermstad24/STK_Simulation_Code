            ...
            for j in self.targets:
                ...
                for i in self.satellites:
                    ...
                    if len(data) > 0:
                        ...
                        self.Azimuth_vs_Elevation[j]+=np.histogram2d(df['Azimuth'].astype(float),df['Elevation'].astype(float), bins=[y,x], density=False)[0]
                    else:
            ...
            for t in self.targets:
                ...
                if len(dfs) > 0:
                    ...
                    for idx,time in enumerate(time_sorted_data['Time']):
                        Target_Angles = np.histogram2d(time_sorted_data['Azimuth'][0:idx].astype(float),time_sorted_data['Elevation'][0:idx].astype(float), bins=[y,x], density=False)[0][0:,:9]
